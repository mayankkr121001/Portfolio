...
function Projects() {
    ...
    return (
        <div id='Projects' className='w-full font-nunito pt-[60px]'>
            <div className='bg-zinc-950 text-center mx-auto pt-[90px] pb-[30px] px-[7%]'>
                <div className='flex gap-2 md:gap-5 justify-center items-center mb-6'>
                    <img className='w-7' src={projectsIcon} alt="project icon" />
                    ...
                    projectsArray.map((elm, index) => ((elm.sNo % 2 == 0 ) ?
                        <div key={index} className=' flex flex-col-reverse items-start lg:flex-row justify-center  gap-[50px] lg:gap-[10%] py-[80px]' >
                            ...
                            <div className='text-left w-[80%] lg:w-[50%] lg:pr-[50px]'>
                                <h1 className='text-2xl sm:text-3xl font-bold'>{elm.sNo}. {elm.name}</h1>
                                <p className='text-[#9e9e9e] text-sm py-4 xl:py-6'>{elm.description}
                                </p>
                                <div className='flex flex-wrap gap-4 xl:gap-8 text-[#fff000]'>
                                    <p>{elm.techStack[0]}</p>
                                    <p>{elm.techStack[1]}</p>
                                    <p>{elm.techStack[2]}</p>
                                    <p>{elm.techStack[3]}</p>
                        ...
                        :
                        <div key={index} className=' flex flex-col-reverse items-start lg:flex-row-reverse justify-center  gap-[50px] lg:gap-[10%] py-[80px]' >
                            <a href={elm.link} target={elm.target} className='projectImageDiv w-[80%] lg:w-[50%]'>
                                <img className='lg:h-[250px] xl:h-auto' ref={(el) => (projectImage.current[index] = el)} onMouseMove={(e) => onImageMouseMoveFunc(e, index)} onMouseLeave={() => onImageMouseLeaveFunc(index)} src={elm.image} alt="weather App" />
                            </a>
                            <div className='text-left w-[80%] lg:w-[50%] lg:pr-[50px]'>
